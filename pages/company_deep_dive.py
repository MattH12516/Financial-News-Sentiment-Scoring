...
with st.expander(f"Add {ticker} to watchlist", expanded=False):
    ...
    if st.button("Add to watchlist", type="primary", key="wl_add"):
        ...
        # Freeze the six-factor score and the individual factors, so the
        # watchlist can compare against them later and name which ones were
        # lost rather than only reporting that the total fell.
        try:
            _pts, _facs = pipeline.current_signal_score(conn, ticker)
            _snapshot["signal_score"] = _pts
            _snapshot["factors"]      = _facs
        except Exception:
            pass
